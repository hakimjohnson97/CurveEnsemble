...
#include "Curve_Libs/JZ_spline_lib.h"
using namespace std;
using namespace JZS;
...
void JZSpline::S_splineFeasible()
{
...
void JZSpline::S_splineFeasibleSmart()
{
    S_spline_feasible_stencil(H, count, coarseRes);
}

void JZSpline::S_splineFeasibleSmart3(int index)
{
    S_spline_feasible_stencil3(H, count, coarseRes, index);
}

void JZSpline::optimizeCurve(double** nodes, int numNodes, InitDir initDir)
{
    int i;
    double theta[coarseRes];
    equi_angular_partition(theta, coarseRes);
...
void JZSpline::fineOptimizeCurve(double **nodes, int numNodes, int depth)
{
     int i;
...
Curve::Node JZSpline::pointInTheMiddle(double *a, double *b)
{
    piece p;
    if (make_piece(a, b, &p) == 0)    {
...
void JZSpline::boundingRect(double &x, double &y, double &width, double &height)
{
