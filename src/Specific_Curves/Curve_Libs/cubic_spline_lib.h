...
// |alpha|, |beta| < pi/2. We find the optimal curve s and
// output Form=1, BE, p1, p2 to FORM8[0], BE8, p18, p28.
//
...
FORM8[0]=1;
if (FORM8[3]==0){ // this indicates the curve is clampled on both sides
  // test whether |alpha|<pi/2
  if (fabs(alpha)>=M_PI_2 || fabs(beta)>=M_PI_2)
    {BE8=INFINITY; printf("S_canonical: error_code=1.\n"); return 1;}
  cos_sin(alpha); p18=s8/c8; cos_sin(beta); p28=s8/c8;
  } // if
else if (FORM8[3]==1) { // this indicates the curve is clampled on the right but free on left
  // test whether |beta|<pi/2
  if (fabs(beta)>=M_PI_2)
    {BE8=INFINITY; printf("S_canonical: error_code=1.\n"); return 1;}
  cos_sin(beta); p28=s8/c8; p18=-0.5*p28;
  } // else if
else if (FORM8[3]==2) {// this indicates the curve is clampled on the left but free on the right
  // test whether |alpha|<pi/2
  if (fabs(alpha)>=M_PI_2)
    {BE8=INFINITY; printf("S_canonical: error_code=1.\n"); return 1;}
  cos_sin(alpha); p18=s8/c8; p28=-0.5*p18;
  } // else if
else {p18=0; p28=0;} // this is the case when the curve is free on both ends
//
BE8=p18*(p18+p28)+p28*p28; return 0;
}// S_canonical--------------------------------------------------------------------------------------------------------
...
if (u2 > alpha_max || u2>=M_PI_2 || v2 > alpha_max || v2>=M_PI_2 )
  return false;
else
  return true;
}// is_feasible_lite----------------------------------------------------------------------------------------
...
//                   BE8, FORM8[3], p18 and p28 (written as outputs)
...
if (error_code==0){
if (FORM8[3]==3)
  {q.form = (short) 0; q.p1=0; q.p2=0; q.alpha=0; q.beta=0;}
else {
  q.form = (short) 1; q.p1 = p18; q.p2 = p28;
  if (FORM8[3]==1)
    alpha=atan(p18);
  if (FORM8[3]==2)
    beta=atan(p28);
...
if (p.form==0)
  ...
  if (k>2*D)
     k=2*D;
  h=1/double(k-1); a1=p.p1+p.p2; a2=p.p1; t=0;
  for (j=0; j<k; j++)
    {x[j]=t; y[j]=t*(t-1)*(a1*t-a2); t+=h;}
}// else
//
