...
struct piece {
  ...
  double p1,p2,q1,q2,alpha,beta; // In Scott's code g1cubicYC04_pi_over_3.m,
  complex c1,c2;                 // vectors V0=[p1,p2] and V1=[q1,q2] are computed
};                               // and passed to hermiteval
...
double den, a0, a1, aa1, aa2, bb1, bb2;
FORM8[0]=1;
if (FORM8[3]==1) { // this indicates the curve is clampled on the right but free on left
  cos_sin(beta); alpha=atan((-c8*s8)/(2-c8*c8));} // if
else if (FORM8[3]==2) { // this indicates the curve is clampled on the left but free on the right
  cos_sin(alpha); beta=atan((-c8*s8)/(2-c8*c8));} // else if
else if (FORM8[3]==3) { // this is the case when the curve is free on both ends
  alpha=0; beta=0;} // else if
//
// test whether |alpha| <= pi/3
if (fabs(alpha)>M_PI_3_PLUS || fabs(beta)>M_PI_3_PLUS)
  {BE8=INFINITY; printf("S_canonical: error_code=1.\n"); return 1;}
cos_sin(beta); c_wig8=c8; s_wig8=s8;
cos_sin(alpha);
temp8=c8*c_wig8 + s8*s_wig8;  // temp8 holds cos(alpha-beta)
den=4-temp8*temp8;   //  den=4-cos(th(1)-th(2))^2;
a0=(6*c8-3*c_wig8*temp8)/den;   // a0=(6*cos(th(1))-3*cos(th(2))*cos(th(1)-th(2)))/den;
a1=(6*c_wig8-3*c8*temp8)/den;   //a1=(6*cos(th(2))-3*cos(th(1))*cos(th(1)-th(2)))/den;
p18=a0*c8; p28=a0*s8; q18=a1*c_wig8; q28=a1*s_wig8;
aa1=6*(p18+q18)-12; aa2=6*(p28+q28);
bb1=6-4*p18-2*q18; bb2=-4*p28-2*q28;
BE8=aa1*bb1+aa2*bb2 + bb1*bb1+bb2*bb2 + (aa1*aa1+aa2*aa2)/3; 
return 0;
}// S_canonical--------------------------------------------------------------------------------------------------------

bool is_feasible_lite (double *u, double *v,double psi,double alpha_max=M_PI_PLUS){
...
//
// The input alpha_max is optional.  If it is not given, then its default value renders it inactive.  It is expected
// that only optimal_direction1 will call is_feasible_lite with alpha_max active (specifically, alpha_max = alpha_max8).
...
if (!v_is_free)
  ...
  FORM8[3] += 2;
u2=fabs(alpha); v2=fabs(beta);
if (u2 > alpha_max || u2>M_PI_3_PLUS || v2 > alpha_max || v2>M_PI_3_PLUS )
  return false;
else
  return true;
}// is_feasible_lite----------------------------------------------------------------------------------------
...
if (FORM8[3]==3)
  ...
  q.form = (short) 1; q.p1 = p18; q.p2 = p28; q.q1=q18; q.q2=q28; q.alpha=alpha; q.beta=beta;
} // else
...
for (k=0;k<N8;k++){
  if (flag_offset)
    ...
    v[DIR]=Theta[k];
  if (is_feasible_lite(u,v,psi_u,alpha_max8)) {
    alpha_u=alpha; beta_u=beta; FORM8_3=FORM8[3];
    if (is_feasible_lite(v,w,psi_v,alpha_max8)) {
