...
#include "Curve_Libs/cubic_spline_lib.h"
using namespace std;
using namespace CubicS;
...
int CubicSpline::getForm(int index)
{
   // return pieces[index].form;
}


void CubicSpline::S_splineFeasible()
{
...
void CubicSpline::S_splineFeasibleSmart()
{
    S_spline_feasible_stencil(H, count, coarseRes);
}

void CubicSpline::S_splineFeasibleSmart3(int index)
{
    S_spline_feasible_stencil3(H, count, coarseRes, index);
}

void CubicSpline::optimizeCurve(double** nodes, int numNodes, InitDir initDir)
{
    int i;
    double theta[coarseRes];
    equi_angular_partition(theta, coarseRes);
...
void CubicSpline::fineOptimizeCurve(double **nodes, int numNodes, int depth)
{
     int i;
...
/*int CubicSpline::save(fstream &file)
{

}

int CubicSpline::load(fstream &file)
{

}

int CubicSpline::saveAsText(fstream &file)
{

}

int CubicSpline::loadFromText(fstream &file)
{

} */

int CubicSpline::S_Curve(double *a, double *b, double *rx, double *ry)
{
...
Curve::Node CubicSpline::pointInTheMiddle(double *a, double *b)
{
    piece p;
    if (make_piece(a, b, &p) == 0)    {
...
void CubicSpline::boundingRect(double &x, double &y, double &width, double &height)
{
